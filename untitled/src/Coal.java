public class Coal extends BattleLoc{
    ...
    public Coal(Player player){
        this.player = player;
        obstacle = new Snake();
        randWiz = new randomWiz(this.player);
    }

    void getLocation(){
        obsPiece = rand.nextInt(3,7);
        while (onLocition){
            int sel;
            if (obsPiece == 0){
                ...
                System.out.println("Karşınızda "+obsPiece+" Adet Yılan var!");
                System.out.println("Karakter sağlık : " + player.healthy);
                System.out.println("Karakter hasar : " + player.damage);
                System.out.println("Yılan sağlık : " + obstacle.obsHealty);
                System.out.println("Yılan hasar : " + obstacle.obsDamage);
    ...
    void war(){
        for(int i = 1; i<= obsPiece;obsPiece--){
            ...
            if (player.healthy <= 0){
                ...
                randWiz.randomWizard();
                if ((obsPiece-1)==0){
                    System.out.println("Tüm canavarlar yok edildi...");
                    obsPiece = 0;
                    break;
                }else {
